...
rlJournalStart
    ...
    for image in $images; do
        ...
        rlPhaseStartTest "Enable EPEL on $image"
            rlRun -s "tmt -vvv run -a plan --name '/crb/enabled' provision --how container --image $image"
        rlPhaseEnd

        rlPhaseStartTest "Disable EPEL on $image"
            rlRun -s "tmt -vvv run -a plan --name '/crb/disabled' provision --how container --image $image"
        rlPhaseEnd

        if is_centos_stream_9 "$image"; then
            rlPhaseStartTest "Check CRB on $image"
                rlRun -s "tmt -vvv run -a plan --name '/crb_package' provision --how container --image $image"
            rlPhaseEnd
        fi
    done
